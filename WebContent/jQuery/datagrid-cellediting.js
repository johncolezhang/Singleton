(function($) {
		...
		if (input) {
			if (accepted) {
				if (dg.datagrid('validateRow', param.index)) {
					dg.datagrid('endEdit', param.index).datagrid('gotoCell',
							param);
				} else {
					input.focus();
				}
			} else {
				dg.datagrid('cancelEdit', param.index).datagrid('gotoCell',
						param);
			}
		} else if (accepted) {
	...
	function getCurrCell(target) {
		var cell = $(target).datagrid('getPanel').find(
				'td.datagrid-row-selected');
		if (cell.length) {
			return {
				index : parseInt(cell.closest('tr.datagrid-row').attr(
						'datagrid-row-index')),
	...
	function unselectCell(target, p) {
		...
		var cell = opts.finder.getTr(target, p.index).find(
				'td[field="' + p.field + '"]');
	...
	function unselectAllCells(target) {
		...
		panel.find('td.datagrid-row-selected').removeClass(
	...
	function getSelectedCells(target) {
		var cells = [];
		var panel = $(target).datagrid('getPanel');
		panel.find('td.datagrid-row-selected').each(
				function() {
					var td = $(this);
					cells.push({
						index : parseInt(td.closest('tr.datagrid-row').attr(
								'datagrid-row-index')),
						field : td.attr('field')
					});
				});
	...
	function gotoCell(target, p) {
		...
		function _go(p) {
			var td = opts.finder.getTr(target, p.index).find(
					'td[field="' + p.field + '"]');
			if (td.length) {
				...
				if (left < 0) {
					body2._scrollLeft(body2._scrollLeft() + left
							* (opts.isRtl ? -1 : 1));
				} else if (left + td._outerWidth() > body2.width()) {
					body2._scrollLeft(body2._scrollLeft()
							+ (left + td._outerWidth() - body2.width())
							* (opts.isRtl ? -1 : 1));
				}
			}
		}
	...
	function enableCellSelecting(target) {
		...
		var dc = state.dc;
		panel.attr('tabindex', 1).css('outline-style', 'none').unbind(
				'.cellediting').bind('keydown.cellediting', function(e) {
	...
	function disableCellSelecting(target) {
		...
		panel.unbind('.cellediting').find('td.datagrid-row-selected')
	...
	function enableCellEditing(target) {
		...
		panel.panel('panel').unbind('.cellediting').bind('keydown.cellediting',
				function(e) {
					e.stopPropagation();
				}).bind('keypress.cellediting', function(e) {
			e.stopPropagation();
	...
	function disableCellEditing(target) {
		...
		opts.onBeforeSelect = opts.OldOnBeforeSelect || opts.onBeforeSelect;
		dg.datagrid('getPanel').unbind('.cellediting').find(
				'td.datagrid-row-selected')
				.removeClass('datagrid-row-selected');
	}
